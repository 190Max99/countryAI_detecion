...
| 模型文件                                      | 用途                           | 获取方式     |
| --------------------------------------------- | ------------------------------ | ------------ |
| `models/scene_resnet18_feature_extractor.pth` | 场景分类特征提取器（ResNet18） | ✅ 已上传 Git |
| `models/scene_classifier_resnet18.pkl`        | 场景分类器（sklearn）          | ✅ 已上传 Git |
| `models/indoor_resnet18.pth`                  | 室内扣分项识别                 | ⚠️ 需自行训练 |
| `models/outside_resnet18.pth`                 | 房前屋后扣分项识别             | ⚠️ 需自行训练 |
| `models/courtyard_resnet18.pth`               | 庭院扣分项识别                 | ⚠️ 需自行训练 |
| `models/courtyard_resnet18_cbam.pth`          | 庭院 CBAM 扣分项识别           | ⚠️ 需自行训练 |
| `models/toilet_resnet18.pth`                  | 厕所扣分项识别                 | ⚠️ 需自行训练 |
| `models/septic_resnet18.pth`                  | 化粪池扣分项识别               | ⚠️ 需自行训练 |

> **说明**：场景分类模型负责判断图片属于室内/庭院/厕所/化粪池/房前屋后中的哪一类，是评分流水线的第一步。该模型体积较小（约 44MB），已直接上传 Git 仓库，clone 后即可使用。其余评分模型需要准备好 `data/all_labels.csv` 后通过训练脚本自行训练生成。
...
在进入具体场景的扣分项识别之前，系统需要先判断每张照片属于哪个场景（室内 / 庭院 / 厕所 / 化粪池 / 房前屋后）。这一步由场景分类模型完成，整体结构为：

```text
输入图片 → ResNet18 特征提取 → sklearn 分类器 → 输出场景类别 + 置信度
```

模型由两部分组成：

- **特征提取器**（`scene_resnet18_feature_extractor.pth`）：基于 ResNet18 的 backbone，去掉最后的全连接层，输出 512 维特征向量
- **分类器**（`scene_classifier_resnet18.pkl`）：sklearn 线性分类器，将 512 维特征映射到 5 个场景类别

在 `train_all_models` 训练流程中，场景分类模型会在训练开始前对 `all_labels.csv` 中所有图片进行预检，标记场景标签与预测不一致的样本，防止人工标注错误导致模型训练偏差。

> **注意**：场景分类模型已上传 Git，clone 后可直接使用。当前仓库不包含该模型的训练脚本，它是在外部标注数据集上训练完成后直接导入使用的。
