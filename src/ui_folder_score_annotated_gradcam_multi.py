...
def make_multilabel_heatmap_overlay(original_image, cam_items, save_path):
    """
    多标签综合 Grad-CAM 热力图。
    ...
    base_rgba = original_image.convert("RGBA")

    overlay_arr = np.zeros((height, width, 4), dtype=np.float32)
    ...
    for item in cam_items:
        ...
        overlay_arr[:, :, 0] += r * cam_arr
        overlay_arr[:, :, 1] += g * cam_arr
        overlay_arr[:, :, 2] += b * cam_arr

        overlay_arr[:, :, 3] = np.maximum(
            overlay_arr[:, :, 3],
            165 * cam_arr
        )

    overlay_arr[:, :, 0:3] = np.clip(overlay_arr[:, :, 0:3], 0, 255)
    overlay_arr[:, :, 3] = np.clip(overlay_arr[:, :, 3], 0, 185)

    overlay_img = Image.fromarray(overlay_arr.astype(np.uint8), mode="RGBA")

    result = Image.alpha_composite(base_rgba, overlay_img)

    panel_h = max(115, 48 + len(cam_items) * 34)

    panel = Image.new("RGBA", (width, panel_h), (255, 255, 255, 230))
    result.alpha_composite(panel, (0, 0))

    draw = ImageDraw.Draw(result)

    font_title = get_chinese_font(size=max(22, width // 45))
    font_text = get_chinese_font(size=max(17, width // 65))
    ...
    for item in cam_items:
        ...
        text = f"{color_name}：{label_name}  概率:{prob:.3f}"
...
def generate_gradcam_images(scene_key, image_path, pred_info, device, save_dir: Path, max_maps=6):
    """
    生成 Grad-CAM 热力图。

    1. 每个预测扣分项分别生成单项热力图；
    2. 如果有多个扣分项，则额外生成一张多标签彩色综合热力图；
    3. 如果没有预测扣分项，则生成最高概率标签的单项热力图。
    """
    ...
    for color_idx, idx in enumerate(indices):
        ...
        save_name = f"{image_path.stem}_{clean_label}_单项热力图.jpg"
        ...
        color, color_name = GRADCAM_COLORS[color_idx % len(GRADCAM_COLORS)]
    ...
    if len(cam_items) >= 2:
        combined_save_path = save_dir / f"{image_path.stem}_多标签综合热力图.jpg"

        make_multilabel_heatmap_overlay(
            original_image=original_image,
            cam_items=cam_items,
            save_path=combined_save_path
        )

        saved_paths.append(combined_save_path)
...
def score_folder(folder: Path, log_fn=None):
    ...
    if label_csv is None:
        raise FileNotFoundError(f"文件夹中没有找到人工标签 CSV: {folder}")
    ...
    for scene_key in ["室内", "庭院", "厕所", "化粪池", "房前屋后"]:
        ...
        if image_path is None:
            ...
            log(f"{scene_key}：已生成文字标注图 {annotated_path}")
...
class ScoreApp:
    ...
    def run_score(self):
        try:
            ...
            self.safe_call(self.log, f"人工标签 CSV：{label_csv}")
            ...
            self.safe_call(self.log, f"文字标注图文件夹：{annotated_dir}")
