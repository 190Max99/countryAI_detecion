...
def annotate_image(scene_key, image_path, pred_info, save_dir: Path):
    ...
    raw_lines = [f"场景：{scene_key}", f"模型扣分：{pred_deduct}"]
...
def score_folder(folder: Path, log_fn=None):
    ...
    for scene_key in ["室内", "庭院", "厕所", "化粪池", "房前屋后"]:
        ...
        if image_path is None:
            ...
            pred_deduct = pred_info["pred_deduct"]
            annotated_path = annotate_image(scene_key, image_path, pred_info, annotated_dir)
    ...
    add_row("室内", 10, scene_deducts["室内"]["true_deduct"], scene_deducts["室内"]["pred_deduct"])
    add_row("庭院", 30, scene_deducts["庭院"]["true_deduct"], scene_deducts["庭院"]["pred_deduct"])
    ...
    add_row("厕所及化粪池", 10, toilet_septic_true, toilet_septic_pred)
    add_row("房前屋后", 10, scene_deducts["房前屋后"]["true_deduct"], scene_deducts["房前屋后"]["pred_deduct"])
...
class ScoreApp:
    def __init__(self, root):
        ...
        title = tk.Label(root, text="AI积分制现场照片评分系统", font=("Microsoft YaHei", 18, "bold"))
        title.pack(pady=12)

        top_frame = tk.Frame(root)
        top_frame.pack(fill="x", padx=20)

        self.folder_label = tk.Label(top_frame, text="当前未选择文件夹", anchor="w", font=("Microsoft YaHei", 10))
        self.folder_label.pack(side="left", fill="x", expand=True)

        choose_btn = tk.Button(top_frame, text="选择农户文件夹", command=self.choose_folder, width=16, font=("Microsoft YaHei", 10))
        choose_btn.pack(side="right", padx=5)

        run_btn = tk.Button(top_frame, text="开始评分", command=self.run_score_thread, width=12, font=("Microsoft YaHei", 10))
        run_btn.pack(side="right", padx=5)

        open_btn = tk.Button(top_frame, text="打开标注图文件夹", command=self.open_annotated_folder, width=16, font=("Microsoft YaHei", 10))
        ...
        self.status_text = tk.Text(root, height=10, font=("Consolas", 10))
        self.status_text.pack(fill="x", padx=20, pady=8)

        self.log("请选择一个农户文件夹，例如 data/raw/97。")
        self.log("评分完成后，会在该文件夹下生成 annotated 文件夹，里面是带扣分项文字标注的图片。")
    ...
    def run_score(self):
        try:
            ...
            result_df, label_csv, save_path, annotated_dir, annotated_paths, device = score_folder(
